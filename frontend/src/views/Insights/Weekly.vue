      ...
      <template v-if="hasInited">
        <v-col cols="12" lg="3" md="4" sm="12" xl="2">
          <InsightsStatsCard
            ...
            :difference="
              report?.data.uploads.total.now -
              report.data.uploads.total.previous
            "
          ...
          <InsightsStatsCard
            ...
            :subtitle="
              $t('insights.statsCards.uploadsPerMonth.subtitle', {
                month: report?.data.uploads.months.series[0].data.reduce(
                  (a, b) => (a.y > b.y ? a : b)
                ).x
              })
            "
    ...
    getReport() {
      ...
      this.axios
        ...
        .then((res) => {
          ...
          // super hacky way to fix the chart not rendering on page init
          setTimeout(() => {
            this.hasInited = true;
          }, 100);
        })
